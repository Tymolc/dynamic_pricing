...
merchant_token = "KTc0pSI43D4aKR4w2NYgnbm9IpwbEldYEHZ4iq9m4CNPdRMfouMcWAloziRET5mh"
...
def trigger_learning(merchant_token, kafka_host):
    fixed_path = make_relative_path("baysian_market_learning.py")
    old_dir = os.getcwd()
    os.chdir(os.path.dirname(fixed_path))
    os.system('python3 -u "{:s}" -t "{:s}" -k "{:s}" >> learning.log &'.format(fixed_path, merchant_token, kafka_host))
    os.chdir(old_dir)
...
merchant_logic = BaysianMerchant()
